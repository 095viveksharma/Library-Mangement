...
void mainmenu()
{
	...
	case '4':
		...
		printf("Programmer....");
		gotoxy(16,4);
		printf("1. vivek sharma)");
		gotoxy(16,5);
		printf("E-mail: 095viveksharma@gmail.com");
		gotoxy(16,7);
		printf("Thanx for using......");
		printf("Please give your feed on the above mentioned email-id.");
		gotoxy(16,8);
		printf("Exiting in 4 second...........>");
		delay(4000);
		exit(0);
	    }
	    default:
...
void deletebooks()    //function that delete items from file fp
{
	...
	if(findbook=='t' )
	{
		...
		fclose(fp);
		remove("vivek.dat");
		rename("test.dat","vivek.dat"); //copy all item from temporary file to fp except that
		fp=fopen("vivek.dat","rb+");    //we want to delete
		if(findbook=='t')
		{
		    gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("vivek.dat","rb+"); //open file for reading propose
